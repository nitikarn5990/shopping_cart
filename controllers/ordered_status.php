...
<div class="col-sm-9">
    <div class="row">
    	<div class="col-sm-12">
        <h1 class="title-bar">รอการชำระเงิน
            <div class="title-border"></div>
        </h1>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                <?php
        $sql = "SELECT * FROM " . $orders->getTbl() . " WHERE  customer_id = " . $_SESSION['customer_id'] . " AND status = 'รอการชำระเงิน' ORDER BY id DESC";
        ...
        while ($row = $db->FetchArray($query)) {
            ?>
                <tr>
                    ...
                    <td><?= $row['status'] ?></td>
                    <td><a href="<?php echo ADDRESS ?>ordered_status/<?= $row['id'] ?>.html" class="btn btn-default">Detail</a></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
